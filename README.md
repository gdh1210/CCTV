<div align="center">
<img src="https://capsule-render.vercel.app/api?type=waving&color=0:83a4d4,100:b6fbff&height=300&&section=header&text=CCTV%20AIoT%20project&fontColor=FFFFFF&stroke=202020&fontSize=90&animation=fadeIn">
</div>
...
* 프로젝트 진행할 시간이 부족하여 서버 구현을 외부프로그램 사용으로 진행하다 보니 통신 관련하여 크게 막히는 점은 없었지만 사용법을 익히고 외부라이브러리 의 동작방식을 이해하려고 노력해 보았지만 내가 작성한 코드가 아니다 보니 완벽한 이해는 어려웠다.
*  같은 네트워크에 있어야 하고 5명의 인원수가 제한되어있는 서버라서 동작가능한 최소한의 기능만을 수행하는 서버라 아쉬웠고(더큰 서버는 돈을 지불해야함) 

---
# 마치며
* 사실 이 프로젝트는 완성이라기 보다는 미완성에 가깝다 기능적인 부분에 집중하느라 세세한 부분에서 구현되지 않은 부분들이 많고
  > 로그인의 화이트리스트 관리, 환경설정
* 컴퓨터 환경은 이리저리 바꿔가면서 많이 테스트 해봤는데 핸드폰 환경은 워낙 다양하다 보니 다른 핸드폰에 적용하면 UI 부분이 많이 틀어지는 것을 확인했고 일부는 고쳤지만 나머지는 수정하지 못했다.
  > 작은 화면에서 버튼 잘림, 큰 화면에서 UI 배치 오류로 버튼 사라짐
